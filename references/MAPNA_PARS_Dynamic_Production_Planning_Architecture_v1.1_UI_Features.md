...
Architecture Baseline v1.1 — Single Source of Truth

24 September 2026 — UI/UX & Features/Functions Architecture Update
...
Architecture Baseline V1.1 adds the detailed Run Simulation / Scenario Workbench UI architecture, Features & Functions catalog, interaction contracts, responsive rules, API mapping and UI acceptance criteria.
...
## 30.3 Run Simulation / Scenario Workbench — Design Authority

صفحه Run Simulation قلب تجربه کاربری سامانه است و باید به‌صورت مستقیم با Scenario Engine، Forecast Engine، Resource Allocation، Cash، Commitment و Decision Governance کار کند. هدف صفحه، تبدیل محاسبات Engine به یک محیط قابل فهم برای تشخیص، شبیه‌سازی، مقایسه و تصمیم انسانی است؛ نه اجرای خودکار تصمیم.

- اصل Priority-First: بحران > هشدار > عادی > تاریخی.
- اصل Progressive Disclosure: لایه صفر برای تشخیص، لایه اول برای علت، لایه دوم برای تحلیل و لایه عمیق برای داده خام/ممیزی.
- Actual immutable است؛ Baseline، Forecast و Scenario با نشانگرهای بصری مجزا نمایش داده شوند.
- Policy و Action دو مفهوم مستقل هستند؛ Policy رفتار Engine را در Scenario تعیین می‌کند و Action یک تغییر صریح و زمان‌دار است.
- هیچ Action حساس بدون Target، Effective Date، Validation و Audit قابل ثبت نیست.

## 30.4 Information Layering

| **لایه**     | **محتوا**                                                                                   | **تعامل**                       |
| ------------ | ------------------------------------------------------------------------------------------- | ------------------------------- |
| Zero Layer   | Critical Alerts، System Health، KPIهای اصلی، وضعیت Scenario و Run                           | مشاهده و ورود به Drill-down     |
| First Layer  | Scenario/Policy/Action، Production Impact، Constraint، Commitment                           | Expand/Drawer/Drill-down        |
| Second Layer | Timeline، Resource Load، Material Forecast، Cash Timeline، Opportunity Cost، Event Timeline | Filter/Zoom/Compare             |
| Deep Layer   | Dependency Graph، Engine Trace، Raw Event Payload، Audit، Run Metadata                      | Inspector/JSON/Replay؛ نقش‌محور |

## 30.5 Visual Structure — Run Simulation

| **ناحیه**            | **محتوا**                                                                                             | **قاعده**                     |
| -------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------- |
| Header               | System, Scenario, Baseline, Simulation Date, Last Calc, Data Quality, User                            | Sticky                        |
| Critical Alerts Zone | Commitment Risk، Blocker، Material/Machine/Manpower/Cash Crisis                                       | Sticky در Crisis              |
| System Health        | Engine State، Data Freshness، Last Successful Run، Latency                                            | همیشه قابل مشاهده             |
| Primary KPI          | At-Risk Commitments، Blocked Cells، Material Shortage، Capacity Gap، Cash Requirement، Decision Queue | Delta نسبت به Baseline اجباری |
| Scenario Workspace   | Scenario، Baseline، Horizon، Policies، Actions، Validation، Run                                       | Action/Policy تفکیک‌شده       |
| Main Workspace       | Production Flow، Cell/PO/Operation، Constraint و Resource Impact                                      | Impact Highlight              |
| Decision Zone        | Alternatives، Feasibility، Opportunity Cost، Approve/Reject/Modify                                    | تصمیم توسط انسان              |

## 30.6 Critical Alerts & System Health

- هر Alert قرمز/کهربایی باید Drill-to-Source داشته باشد: Commitment → Product/Project → Cell → PO → Operation → Constraint → Resource → Event.
- Alertها باید Deduplicate شوند و Severity مشخص داشته باشند.
- System Health شامل Engine Status، Data Freshness، Last Run و Response/Queue Status است.
- در داده Stale، Timestamp آخرین داده معتبر و وضعیت Stale صریح نمایش داده شود.

## 30.7 Primary KPI Contract

| **KPI**             | **معنا**                                | **Drill-down**      |
| ------------------- | --------------------------------------- | ------------------- |
| At-Risk Commitments | تعهدات در معرض تأخیر                    | Commitment Trace    |
| Blocked Cells       | Cellهای دارای Constraint فعال           | Constraint Explorer |
| Material Shortage   | ماده و تاریخ اولین کمبود                | Material Forecast   |
| Capacity Gap        | ساعت/شیفت کمبود ظرفیت                   | Capacity Board      |
| Cash Requirement    | نیاز نقدینگی و زمان آن؛ بدون خرج خودکار | Cash Timeline       |
| Decision Queue      | تصمیم‌های باز و Critical Count          | Decision Workbench  |

## 30.8 Scenario Definition, Policies & Actions

| **Feature**        | **Function**                                                 | **Rule**                                     |
| ------------------ | ------------------------------------------------------------ | -------------------------------------------- |
| Scenario Selector  | انتخاب Scenario، نسخه و هدف                                  | Context تمام صفحه را تعیین می‌کند            |
| Baseline Selector  | انتخاب Snapshot immutable                                    | هر Run یک baseline_snapshot_id دارد          |
| Horizon Control    | تنظیم Start/End                                              | Start≤End و تاریخ معتبر                      |
| Policy Editor      | Resource Allocation / Commitment / Cash / Capacity           | Scenario-based و versioned                   |
| Action Builder     | Pause / Resume / Cancel / Cash Injection / Additional Shift  | Target + Effective Date + Validation + Audit |
| Pre-Run Validation | کنترل Snapshot، Policy، Action، Data Quality و Authorization | Run در خطای بحرانی شروع نشود                 |

## 30.9 Action Semantics in UI

| **Action**       | **UI Behavior**                       | **Engine Meaning**                                       |
| ---------------- | ------------------------------------- | -------------------------------------------------------- |
| Pause            | توقف و نمایش Resource Release         | از تاریخ مؤثر، اجرای جدید متوقف؛ منابع مصرف‌نشده آزاد    |
| Resume           | دکمه Re-queue                         | ورود مجدد به Allocation Engine؛ Restore منابع قبلی ممنوع |
| Cancel           | تأیید دو مرحله‌ای                     | توقف دائمی؛ منابع مصرف‌نشده آزاد؛ Resume ممنوع           |
| Cash Injection   | ثبت به‌عنوان Decision/Scenario Action | فقط Cash State را تغییر می‌دهد؛ خرج خودکار ممنوع         |
| Additional Shift | ثبت شیفت و ظرفیت/هزینه                | V1 ظرفیت افزوده از Shift؛ خرید ماشین خارج از V1          |

## 30.10 Simulation Run Experience

| **State**          | **UI**                                     | **Required Data**              |
| ------------------ | ------------------------------------------ | ------------------------------ |
| Queued             | در صف                                      | run_id, queue state            |
| Validating         | اعتبارسنجی                                 | validation result              |
| Running            | Progress + simulation date + latest event  | run_id, progress, current date |
| Impact Calculation | محاسبه Production/Resource/Cash/Commitment | partial/complete metrics       |
| Comparing          | Baseline vs Scenario                       | delta set                      |
| Completed          | Result + Decision Case entry               | final result                   |
| Failed             | Cause + Trace + Retry                      | error code, correlation id     |

## 30.11 Production / Cell / Resource Views

- Product/Project → Cell/PO → Operation به‌صورت سلسله‌مراتبی و Dependency-aware نمایش داده شود.
- Cell وضعیت READY/RUNNING/BLOCKED/COMPLETED/PAUSED/CANCELLED داشته باشد.
- Constraint Explorer باید Material، Machine، Manpower، Cash و Dependency را تفکیک کند.
- Resource Impact باید Release، Previous Owner، New Owner، Idle Days و Opportunity Cost را قابل مشاهده کند.

## 30.12 Analytical Views

| **View**                        | **Purpose**                                            |
| ------------------------------- | ------------------------------------------------------ |
| Baseline vs Scenario Completion | اثر Scenario بر خروجی تولید                            |
| Commitment Timeline             | تغییر Forecast Date نسبت به Commitment                 |
| Resource Load                   | بار Machine/Manpower/Material/Cash در طول زمان         |
| Material Forecast               | Available/Reserved/Consumed/Projected و اولین Shortage |
| Cash Timeline                   | Requirement/Gap/Injection timing                       |
| Opportunity Cost                | Gain/Loss/Displacement ناشی از تخصیص منابع             |
| Event Timeline                  | Action/Event/State Change/Constraint                   |

- تمام نمودارها باید Actual، Baseline، Forecast و Scenario را از نظر بصری و Legend متمایز کنند.

## 30.13 Decision & Impact Workbench

Scenario Result باید به مجموعه‌ای از اثرات و Alternativeهای عددی تبدیل شود. Engine Alternative را ارزیابی می‌کند؛ انسان تصمیم نهایی را می‌گیرد.

| **بخش**          | **Function**                                                        |
| ---------------- | ------------------------------------------------------------------- |
| Impact Summary   | Production، Commitment، Resource، Material، Machine، Manpower، Cash |
| Opportunity Cost | منابع آزادشده، مصرف‌کننده جدید، محصول جابه‌جا شده، Gain/Loss        |
| Alternatives     | Feasible/Not Feasible + معیارهای عددی                               |
| Decision         | Approve / Reject / Modify با Audit                                  |
| Commit Plan      | تبدیل تصمیم تأییدشده به Plan با کنترل نسخه و مجوز                   |

## 30.14 Features & Functions Catalog

| **ID** | **Feature / Function**  | **شرح**                        |
| ------ | ----------------------- | ------------------------------ |
| UI-F01 | Scenario Selection      | انتخاب Scenario و Version      |
| UI-F02 | Baseline Snapshot       | انتخاب Snapshot immutable      |
| UI-F03 | Horizon Control         | بازه شبیه‌سازی                 |
| UI-F04 | Policy Editor           | Policyهای Scenario             |
| UI-F05 | Action Builder          | Pause/Resume/Cancel/Cash/Shift |
| UI-F06 | Pre-Run Validation      | اعتبارسنجی پیش از Run          |
| UI-F07 | Run Simulation          | شروع و پایش Engine             |
| UI-F08 | Critical Alert Center   | بحران و هشدار                  |
| UI-F09 | KPI Strip               | شش KPI اصلی                    |
| UI-F10 | Production Flow         | Product/Cell/PO/Operation      |
| UI-F11 | Constraint Explorer     | علت Blocker                    |
| UI-F12 | Resource Board          | بار و Reallocation             |
| UI-F13 | Material Forecast       | پیش‌بینی کمبود                 |
| UI-F14 | Cash Timeline           | نیاز نقدینگی و زمان            |
| UI-F15 | Commitment Trace        | Trace تا Constraint            |
| UI-F16 | Opportunity Cost        | Gain/Loss/Displacement         |
| UI-F17 | Baseline vs Scenario    | مقایسه                         |
| UI-F18 | Decision Case           | Alternative comparison         |
| UI-F19 | Decision Approval       | Approve/Reject/Modify          |
| UI-F20 | Event Timeline          | Trace رویدادها                 |
| UI-F21 | Audit Inspector         | Before/After/Actor             |
| UI-F22 | Engine Trace            | Trace محاسبات                  |
| UI-F23 | Replay                  | بازپخش Run/Scenario            |
| UI-F24 | Saved View              | ذخیره Context                  |
| UI-F25 | Responsive Control Room | Desktop/Tablet/Mobile          |
| UI-F26 | Data Quality            | Freshness/Validity             |
| UI-F27 | Notification Center     | Severity-aware                 |
| UI-F28 | Global Search           | Project/Cell/PO/Material/Run   |

## 30.15 Responsive & Control-Room Rules

- Desktop حداقل ۱۲۸۰ px و شبکه ۱۲ ستونه؛ Tablet پنل‌های جزئیات را Drawer کند؛ Mobile فقط Priority-First ترتیب را حفظ کند.
- ترتیب Mobile: Critical Alerts → System Health → KPI → Scenario Status → Actions → Decision Summary → Details.
- کنترل‌های بحرانی حداقل ۴۴×۴۴ px و Actionهای مخرب دارای Confirmation باشند.
- Red/Green تنها حامل معنا نباشند؛ Icon/Label/Shape نیز وضعیت را منتقل کنند.
- Animation غیرضروری حذف شود تا بار شناختی و خستگی شیفتی کاهش یابد.

## 30.16 UI Data Contract & API Mapping

| **UI Component**  | **API**                                  | **Trace Key**            |
| ----------------- | ---------------------------------------- | ------------------------ |
| Scenario          | GET /api/v1/scenarios                    | scenario_id/version      |
| Actions           | POST /api/v1/scenarios/{id}/actions      | action_id                |
| Run               | POST /api/v1/simulation-runs             | run_id                   |
| Run Result        | GET /api/v1/simulation-runs/{id}         | run_id                   |
| Resources         | GET /api/v1/resources                    | resource_id/date         |
| Material Forecast | GET /api/v1/materials/{id}/forecast      | material_id/date         |
| Commitment        | GET /api/v1/commitments/{id}             | commitment_id            |
| Decision Case     | GET/POST /api/v1/decision-cases          | decision_case_id         |
| Approval          | POST /api/v1/decision-cases/{id}/approve | decision_id              |
| Cash              | GET /api/v1/cash/requirements            | cash_requirement_id/date |
| Audit             | GET /api/v1/audit-events                 | event_id/correlation_id  |

- UI مستقیماً به Database متصل نشود. Mutationها از API/BFF عبور کنند و correlation_id، user context، scenario_id و run_id را منتقل کنند.

## 30.17 UI Acceptance Criteria

| **ID**  | **Acceptance Criterion**                                             |
| ------- | -------------------------------------------------------------------- |
| UX-AC01 | Critical Alert و System Health در اولین نگاه قابل تشخیص باشند.       |
| UX-AC02 | هر KPI بحرانی Drill-to-Source داشته باشد.                            |
| UX-AC03 | Baseline و Scenario همیشه با Delta روشن مقایسه شوند.                 |
| UX-AC04 | Run بدون ورودی‌های اجباری معتبر شروع نشود.                           |
| UX-AC05 | Action قبل از ثبت Validation و Audit داشته باشد.                     |
| UX-AC06 | Pause/Cancel Resource Release را قابل Trace کنند.                    |
| UX-AC07 | Resume به‌صورت Re-queue نمایش داده شود.                              |
| UX-AC08 | Cash Injection هرگز به‌صورت پرداخت خودکار نمایش داده نشود.           |
| UX-AC09 | Scenario اثر خود را بر Production/Commitment/Resource/Cash نشان دهد. |
| UX-AC10 | هر Alternative به run_id متصل باشد.                                  |
| UX-AC11 | Failure دارای علت دقیق و Trace باشد.                                 |
| UX-AC12 | Mobile کنترل‌های بحرانی را از دسترس خارج نکند.                       |
| UX-AC13 | UI منطق Engine را حدس یا بازتعریف نکند.                              |
| UX-AC14 | Mutationهای حساس Audit شوند.                                         |

## 30.18 Layout Pseudocode
...
MAIN_WORKSPACE { ProductionFlow; ConstraintPanel; ResourceImpact; OpportunityCost }

ANALYSIS_ZONE { BaselineVsScenario; Timeline; MaterialForecast; CashTimeline; CommitmentImpact }

DECISION_ZONE { Alternatives; Feasibility; Impact; Approve/Reject/Modify }
